# SplitEven — Shared Expense & Budget App

> **Name:** SplitEven *(renamed from the working name "EvenSplit" on 2026-08-25 — see Progress Log)*
> **Status:** 🟢 Phases 0–6 built and live, including the mobile Android EAS build; only screenshots remain — see Progress Log
...
Rebranded on 2026-08-25 from an earlier deep-teal-green (`#2F6F5E`) placeholder to the SplitEven palette supplied directly by Sej:

| Token | Hex | Usage |
|---|---|---|
| `primary` (bright) | `#35D6B5` | Dark-mode primary, brand accent |
| `primary` (dark variant) | `#16A88F` | Light-mode primary (AA contrast on white) |
| `primary` (light/soft variant) | `#63E7CE` | Dark-mode highlights, gradient stop |
| `dark background` | `#0A0A0A` | Dark-mode app background, logo background |
| `positive` | `#2E9E6B` | "You are owed" states (unchanged) |
| `negative` | `#D95F5F` | "You owe" states (unchanged) |
| `neutral-900` | `#0A0A0A` | Primary text (light mode) / dark-mode background |
| `neutral-500` | `#6B7169` | Secondary text (unchanged) |
| `neutral-100` | `#F4F5F3` | Light-mode app background (unchanged) |
| `surface` | `#FFFFFF` (light) / `#141414`–`#22271F`→`#141414` (dark) | Cards |
| `warning` | `#E0A63A` | Pending/unsettled flags (unchanged) |

Implemented in `apps/web/src/app/globals.css` (CSS custom properties, light + dark) and `apps/mobile/tailwind.config.js` (`colors.primary.{DEFAULT,light,bright,soft}`, `colors.neutral.900`).

### 3.3 Typography
- **Font:** Poppins everywhere — web (`next/font/google`) and mobile (`@expo-google-fonts/poppins`, replacing an earlier Inter placeholder on 2026-08-25), with JetBrains Mono kept specifically for tabular currency figures.
...
SplitEven's mark is an "S/E" monogram — the teal "S" flows into a diagonal cut, with a staggered white "E" on the other side — designed by Sej directly (not AI-generated by this codebase) and supplied as a transparent PNG.

- **Master source:** `apps/web/public/logo-mark.png` (transparent, ~500px, keep this as the source of truth for regenerating anything below).
- **Web:** `apps/web/public/logo.png` (composited onto `#0A0A0A`, used by the `<Logo>` component in the nav bar and login screen) and `apps/web/src/app/icon.png` (same composite, Next.js favicon convention).
- **Mobile:** every icon slot in `apps/mobile/assets/` regenerated from the master — `icon.png` and `favicon.png` (composited onto `#0A0A0A`), `android-icon-foreground.png` (the transparent mark directly, sized for the adaptive-icon safe zone), `android-icon-background.png` (solid `#0A0A0A`), `android-icon-monochrome.png` (true alpha-derived white silhouette, not a color-threshold guess — those don't work when a design has white elements against a white export background, which this did), and `splash-icon.png` (transparent, composited live by `expo-splash-screen` over its configured `#0A0A0A` background).
...
**Mobile:** Explicitly **not** the standard admin/back-office mobile pattern used on StocklaneOS, TMS, or PGB Logistics (dense tables, form-heavy screens, utilitarian nav). SplitEven mobile should read as **consumer fintech** — closer to Splitwise / Cash App / Revolut:
...
- **2026-08-25** — **Critical bug found and (attempted) fix**: the app crashed on every navigation past login/profile-setup, on a real Android emulator, for the first time anyone actually tested that path end-to-end (every prior "verification" in this project was `tsc`/build/static-analysis only). Crash: `com.facebook.react.common.JavascriptException: TypeError: Object cannot be used as a constructor` inside a class called `LRUCache`, reached via expo-router's route-tree-building internals (`loadRoute` → `getDirectoryTree` → `getRoutes`). Root-caused via a proper Android SDK + emulator set up on this machine (`C:\Android`, `ANDROID_HOME`/`JAVA_HOME` env vars, an AVD named `SplitEven_Test`) and `adb`/`uiautomator` for on-device debugging — genuinely reproduced and inspected, not guessed. Investigation trail: ruled out duplicate React/react-native-screens versions; confirmed 4 different `lru-cache` npm versions exist in the monorepo but all in build-only tooling (Babel/Metro/eas-cli devDependencies, per `pnpm why`), never reaching the app bundle; extracted the *actual* Hermes bytecode straight out of the installed APK (`assets/index.android.bundle`) and confirmed the literal string `LRUCache` really is baked into that specific build; regenerated the bundle locally from the identical git commit twice (including after fixing a stale local `node_modules` that `pnpm install --frozen-lockfile` silently repaired) and could **not** reproduce `LRUCache` appearing at all — meaning the crash is specific to something about the EAS cloud (Linux) build environment that doesn't reproduce in local Windows Metro bundling, and the exact root mechanism was not conclusively identified.
  - **Fix attempted**: disabled `"experiments": { "typedRoutes": false }` in `app.json` — the crash trace pointed directly at expo-router's route-tree/typed-route-building code, and this is the most surgical, lowest-risk change available. `tsc --noEmit` still clean (runtime navigation doesn't depend on typed-route generation, only compile-time path autocomplete does).
  - **Verified against a real device on 2026-08-25 (later same day): fix did NOT work, crash still reproduces.** Built a fresh EAS APK with `typedRoutes: false`, confirmed pre-install that the literal string `LRUCache` was still present in the extracted Hermes bundle, then installed and drove the app end-to-end on the emulator through a real login → profile-setup → tab navigation flow. App crashed out to the Android home screen at the exact same point, with the identical signature: `TypeError: Object cannot be used as a constructor` at `LRUCache@1:1813056`, called via `reset()`, reached through `getRoutes → getDirectoryTree → getLayoutNode → loadRoute → metroContext → ContextNavigator` (same call chain as before). Along the way, also chased down and ruled out a red herring: grepped every `node_modules` package for a literal `class LRUCache`/`function LRUCache` declaration and the *only* match anywhere in the tree is inside `react-devtools-core/dist/backend.js` (a dev-only, unrelated package) — meaning the `LRUCache` name in the crash stack is very likely a Hermes stack-symbolication artifact (nearest named function in a sourcemap-less trace), not a real pointer to that file. The actual root cause is still not conclusively identified. **Next step, not yet started**: get a `development`-profile EAS build (dev client) so a live JS debugger can attach and get a properly symbolicated stack trace instead of guessing from cold logcat output — static grepping across the dependency tree has been exhausted without a conclusive answer.
  - **Unrelated finding, now fixed**: while retesting, hit a second, *different* bug blocking login on the test account — Supabase auth logs showed `sql: Scan error on column index 3, name "confirmation_token": converting NULL to string is unsupported`. Root cause: a test account created earlier in this session via raw SQL `INSERT` into `auth.users` never set the token columns (`confirmation_token`, `recovery_token`, etc.), which defaulted to `NULL`; GoTrue's Go scanner can't read a `NULL` into those non-nullable string fields, so *any* login attempt on that account failed with "Database error querying schema" — a bug in the test data, not the app. Fixed the existing row (`UPDATE ... SET confirmation_token = '', ...`), and for the retest created a second account the correct way — through Supabase's real `/signup` REST endpoint rather than raw SQL — which avoids this whole bug class entirely. Confirmed via direct API call (`/auth/v1/token`) that the fixed credentials work outside the app too, isolating this as a data problem, not a backend/API problem.
- **2026-08-25** — Mobile navigation and content expansion, per Sej's feedback that the app felt empty next to reference fintech apps (GCash, a Maya-style credit screen, a MyMoney-style ledger) that all share a pattern: a labeled bottom nav with 4-5 tabs plus a quick-action icon grid above the main content. Concretely: expanded the tab bar from 2 tabs (Groups, Settings) to 4 (Home, **Activity**, **Insights**, Settings) — Activity and Insights were previously buried inside each individual group's detail screen only; they're now aggregated top-level views across every group the user belongs to (`fetchActivityForGroups`/`fetchExpensesForGroups` in `lib/api`, `useAllActivity`/`useAllExpenses` hooks). Insights deliberately keeps spending-by-category totals segmented **per currency** rather than blending a PHP group and a USD group into one misleading number. Added a `QuickActions` icon grid to the Home tab (New group, Join group, Activity, Insights) plus a `JoinGroupSheet` for entering an invite code directly instead of only supporting shared links. Verified: `tsc --noEmit` clean, shared tests still 21/21.
  - **Feature ideas suggested but not built this pass** (flagged for Sej to prioritize, not unilaterally scope-crept): personal/non-group expense tracking outside of shared groups, per-category budgets with over-budget warnings, recurring bill reminders as push notifications (the recurring-expense *materialization* already exists per Phase 6, but nothing surfaces "this is due soon" proactively), receipt OCR to auto-fill expense amount/category, multi-currency conversion display (today currencies are shown separately, never converted/summed), a linked-account/bank-import style view. None of these are implemented — noted here so the decision of what's next is explicit rather than silently deferred.

- **2026-08-25** — Full rebrand: EvenSplit → **SplitEven**. Scope decision: renamed everything user-visible (page titles/metadata, nav/login/settings copy, mobile app display name and OS permission text, README/PROJECT_PLAN/HANDOFF docs, GitHub repo), but deliberately left internal-only identifiers alone — the `@evensplit/shared` npm workspace package name (~40 import sites, zero user-visible payoff to touch), the Supabase project's internal name, and the EAS project slug/projectId — to avoid a large mechanical refactor with no visible benefit. New color palette applied to both apps (`#35D6B5`/`#16A88F`/`#63E7CE` primary family, `#0A0A0A` dark background, see §3.2). New logo: after two rounds of a hand-drawn SVG recreation (documented misstep worth noting — the first attempt used literal "S"/"E" text glyphs, the second hand-drew a closer flowing shape but was still an approximation), Sej supplied the actual logo as a PNG. First export was flattened on solid white with no alpha channel, discovered via `sharp` metadata (`hasAlpha: false`) — correctly declined to auto-remove that background myself, since the design's white "E" bars are the same color as the white background and a naive cutout would have eaten them too. Sej then supplied a properly transparent re-export (confirmed `hasAlpha: true` before using it), which became the master (`apps/web/public/logo-mark.png`). Regenerated every icon/favicon slot for both apps from that master (see §3.3a), including a true alpha-derived monochrome for Android rather than a broken luminance-threshold guess. Landing page also got 3 new content sections (How it works, expanded FAQ) beyond the earlier redesign pass, per the "add more content" ask. Verified: `tsc --noEmit` clean on both apps, `pnpm --filter web build` clean, favicon/icon routes confirmed in the build output, composited icon assets visually inspected directly (browser screenshot tool was intermittently unavailable this session, so verification leaned on direct image reads instead where the browser didn't cooperate).
