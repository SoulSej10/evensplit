...
- **2026-08-25** — Mobile UI pass per Sej's reference (a dark fintech app screenshot): the concrete, actionable ask was "buttons aligned at the bottom" rather than a full dark-theme overhaul, so scoped to that. New `BottomActionBar` component (`src/components/ui/BottomActionBar.tsx`) — a safe-area-aware bar pinned to the screen bottom — applied to Login, Signup, Profile Setup (primary CTA moved out of scrolling content into the fixed bar) and the Group Detail screen (replaced the single floating "Add Expense" FAB with a two-button bar: "Settle up" — switches to the Balances tab since settling needs a specific from/to pair, disabled when already settled — and "Add expense", echoing the reference's twin-button pattern). Also completed the earlier "Poppins overall" instruction, which had only been applied to web: swapped mobile from Inter to Poppins (`@expo-google-fonts/poppins` installed, Inter package removed, `tailwind.config.js` fontFamily + `app/_layout.tsx` font loading updated) and tightened the mobile `card` border-radius token from 20px to 14px to match the web radius fix. Verified: `tsc --noEmit` clean.

- **2026-08-25** — EAS Android builds: first preview build (`ebe42cb7`) succeeded but predates both the Supabase EAS env vars and the mobile UI pass above, proving the signing/keystore pipeline works without yet connecting to Supabase or having the new UI. Registered `EXPO_PUBLIC_SUPABASE_URL`/`EXPO_PUBLIC_SUPABASE_ANON_KEY` as EAS project environment variables (plaintext visibility, not secrets) across production/preview/development. Second build (`2d49289f`) confirmed those env vars load correctly but still predates the UI pass. A third build with everything current is the one worth actually installing. iOS not attempted, needs an Apple Developer account. Also created a pre-confirmed test account (`admin@evensplit.test`, seeded with 2 mock members and 2 groups of realistic expense/settlement data) directly via Supabase MCP so Sej can navigate the live app without depending on email confirmation.
